...
def timer():
    ...
    scope = "user-library-read playlist-read-private playlist-modify-private user-modify-playback-state user-read-playback-state"
    global sp
    sp = spotipy.Spotify(
...
def play():
    ...
    difference_to_timer = float(
        abs(results["items"][0]["track"]["duration_ms"] - float(timer))
    )
    play_track_index = 0
    for count, track_results in enumerate(results["items"]):
        current_track_difference = float(
            abs(track_results["track"]["duration_ms"] - float(timer))
        )
    ...
    while (
        float(timer) > float(playlist_length)
        and float(timer) - float(playlist_length) > 15000
    ):  # 15 seconds
        ...
        playlist_length += float(
            results["items"][play_track_index]["track"]["duration_ms"]
        )
    ...
    #################################################################
    # Get a device to play on, first see if active device is availble if not then just
    ...
    if len(userDevicesList) > 1:
        for device in userDevicesList:
            if device["is_active"] == "true":
                deviceIDToPlay = device["id"]
                break

    deviceIDToPlay = selectedDevice["id"]

    #################################################################
    # Play Song
    playlistUri = "spotify:playlist:" + timerPlaylistId
    result = sp.start_playback(device_id=deviceIDToPlay, context_uri=playlistUri)
...
def countdown():
    return render_template(
        "countdown.html", title="Music Timer for Spotify", timer=timer
    )
